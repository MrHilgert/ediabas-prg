...
## VM — полная реализация (идемпотентность для любых ЭБУ)

- [x] **Декодер инструкций доказан на корпусе** — 1748 ЭБУ, 101 598 jobs, 99.92%
      чистого декода до `eoj`. Спец: `enewset(0x40)`=2 байта. См. `BEST2-DECODER.md`.
      Единая функция длины операнда по нибблу mode → поток не рассинхронивается.
- [x] **Float-арифметика** `a2flt/fix2flt/fadd/fsub/fmul/fdiv/fcomp/flt2fix/flt2a`
      с выставлением флагов (`Value::Float`, хранение в S-регистрах). ✓ 2026-07-01
      Проверено вживую: `STATUS_ARMM_LIST` → **480 mg/Hub Luft** на реальном DDE4.0.
      (`STATUS_DZMNMIT` даёт ERROR при RPM=0 на столе — это верная логика job'а.)
- [ ] Аудит оставшихся `skip_instr`-заглушек — реализовать значимую семантику
- [ ] Регресс-тест: прогон Rust-декодера по всему `ecu/` в CI (гарантия «всегда»)
- [ ] Парсинг DTC в именованные результаты (FS_LESEN читает, но не форматирует)
...
- [x] **ЭБУ отвечает на DS2 фрейм** ✓ РЕШЕНО 2026-07-01 (Windows, COM3, DDE40KW0)

> **Причина «молчания» ЭБУ:** отсутствие межбайтовой паузы при TX.
> DDE4.0 (и классика DS2) игнорирует кадр, если байты идут без ~4-5 мс интервала
> (ParInterbyteTime). Эхо при этом всегда проходит (простое отражение трансивера),
> из-за чего казалось «связь есть, но ответа нет». KDCAN/FTDI **эхоит** TX — вопреки
> старой заметке в CLAUDE.md; эхо надо дренировать (`--echo`).
>
> **Фикс:** `interbyte_ms` в `Ds2Transport::send_raw` (побайтный TX с паузой),
> проброшен в `CommConfig`, `configure()`, команды `raw`/`run`. Дефолт 5 мс
> (надёжнее 4 из-за гранулярности Windows `Sleep`).
>
> Рабочая проверка:
> ```
> ediabas-prg raw --port COM3 --baud 9600 --echo --interbyte 5 "B8 12 F1 01 A2"
> → RX 48 байт, CHK OK, ID_BMW_NR="7789376" ID_ZYKLUS="C029"
> ```
> **TODO (research):** взять interbyte из .prg (CommParameter[8]=4 в INITIALISIERUNG
> DDE40KW0 совпадает по значению — подтвердить, что это именно ParInterbyteTime,
> а не hdr_len).
