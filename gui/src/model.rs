...
/// Категория сообщения середины сессии — задаёт и вид в UI, и уровень лога.
/// (`link/` знает `ediabas::Error::is_transport`; `ui/` — только эти два случая.)
pub enum NoticeKind {
    /// Шина/ЭБУ: таймаут, чек-сумма, протокол, отказ ЭБУ. Ожидаемый транзиентный шум —
    /// тихая строка статуса, тихий лог (транспорт и так трассируется в ediabas-слое).
    Bus,
    /// Внутренняя: интерпретатор (`Vm`), разбор `.prg`, неподдержанное. Дефект инструмента
    /// или данных SGBD — показываем заметно и логируем громко (это не «машина виновата»).
    Internal,
}
...
pub enum Update {
    ...
    /// Опрос спросил шину и не получил ответа (транзиентный сбой) — не фатально, но
    /// повторяясь подряд, это признак потери связи. НЕ шлётся, когда опрашивать нечего
    /// (у экрана нет джобов) или джоб упал внутренней ошибкой — иначе линк рвётся зря.
    PollMiss,
    /// Сообщение середины сессии (ошибка одного джоба). Связь держим всегда; `kind`
    /// разводит шинный шум и внутренний дефект (см. [`NoticeKind`]).
